...
- [ ] `booking_rules.py`: генерация слотов по сетке 30 мин в пределах 09:00–17:00 так, что `start + durationMinutes <= 17:00`, в окне 14 дней, исключая прошлое и слоты, пересекающиеся с существующими бронированиями (глобально)
- [ ] `GET /event-types/{eventTypeId}/slots?date=` → 200 Slot[]; 404 при неизвестном eventTypeId; фильтр по одному дню через `date`
- [ ] Тесты: корректная сетка/границы часов, исключение прошлого и слотов вне окна, исключение занятых слотов, фильтр по `date`, 404 на несуществующий event type
- [ ] Прогнать pytest — должен пройти перед Task 5
...
- [ ] Прогнать полный тест-набор (`pytest` из `backend/`)
- [ ] Прогнать линтер, если настроен (ruff); иначе пропустить с пометкой
- [ ] Проверить покрытие ≥80% (`pytest --cov`)
- [ ] Убедиться, что сервер запускается (`uvicorn app.main:app`) и эндпоинты отвечают согласно контракту
